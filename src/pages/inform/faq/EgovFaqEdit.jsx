...
function EgovFaqEdit(props) {
  console.group("EgovFaqEdit");
  console.log("------------------------------");
  console.log("EgovFaqEdit [props] : ", props);

  const navigate = useNavigate();
  const location = useLocation();
  console.log("EgovFaqEdit [location] : ", location);
  //관리자 권한 체크때문에 추가(아래)
    ...
    if (bbsFormVaildator(formData)) {
      ...
      EgovNet.requestFetch(modeInfo.editURL, requestOptions, (resp) => {
        if (Number(resp.resultCode) === Number(CODE.RCV_SUCCESS)) {
          navigate(URL.INFORM_FAQ, { state: { bbsId: bbsId } });
        } else {
